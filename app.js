...
async function getGoogleToken() {
  ...
  const stored = sessionStorage.getItem('goog_token');
  const expiry = Number(sessionStorage.getItem('goog_token_exp') || 0);
  if (stored && Date.now() < expiry) return stored;

  // Start OAuth redirect flow
  const CLIENT_ID = localStorage.getItem('googleClientId') || '';
  if (!CLIENT_ID) {
    // Ask user for client ID
    const id = prompt('Google Client ID girin (console.cloud.google.com):\n(Boş bırakırsanız takvim özelliği çalışmaz)');
    if (!id) throw new Error('Google Client ID girilmedi');
    localStorage.setItem('googleClientId', id);
  }
  const cid = localStorage.getItem('googleClientId');
  const redirect = encodeURIComponent(window.location.origin + window.location.pathname);
  const scope = encodeURIComponent('https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/drive.file');
  const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${cid}&redirect_uri=${redirect}&response_type=token&scope=${scope}&prompt=consent`;
  
  // Store state and redirect
  sessionStorage.setItem('oauth_return', 'calendar');
...
function handleOAuthCallback() {
  ...
  if (token) {
    sessionStorage.setItem('goog_token', token);
    sessionStorage.setItem('goog_token_exp', String(expires));
    // Clean URL
    history.replaceState(null, '', window.location.pathname);
    toast('✅ Google hesabı bağlandı', 'success');
    return true;
  }
  return false;
}
...
async function driveBackupDirect(token, dataStr) {
  try {
    const filename = `deneme-takip-yedek-${new Date().toISOString().split('T')[0]}.json`;
...
async function driveBackup() {
  try {
    ...
    const result = await driveBackupDirect(token, JSON.stringify(data));
    if (result?.ok) {
      state.settings.lastBackup = Date.now();
      await saveData('settings');
      renderSettings();
